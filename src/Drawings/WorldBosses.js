...
class WorldBosses {
    ...
    listToDiscord(data, user, asEmbed = false) {
        ...
        if (asEmbed) {
            ...
            ListOfWorldBosses.load({ collection: data.bosses, displayIfEmpty: Translator.getString(lang, "world_bosses", "no_world_boss"), listType: 0 }, lang, (index, info) => {
                let nextWorkBossInfo = Emojis.general.national_park + " " + info.regionName + " - " + info.areaName + " (" + info.idArea + ")\n\n";
    ...
    lastBossStats(data) {
        ...
        if (data.worldBoss != null) {
            let embed = new GenericMultipleEmbedList();
            embed.load({
                collection: [Translator.getString(lang, "world_bosses", "boss_fight_recap_damage_dealt", [data.worldBoss.damage, data.worldBoss.damageRank]), Translator.getString(lang, "world_bosses", "boss_fight_recap_attack_count", [data.worldBoss.attackCount, data.worldBoss.attackCountRank])],
                listType: 0,
            },
                lang,
                (index, content) => {
                    return Emojis.general.crossed_swords + " " + content;
                }
            )
            return embed.getEmbed(new Discord.MessageEmbed().setTitle(data.worldBoss.bossName));
        } else {
